...
## 三、 迭代器与"前闭后开"区间
### 1. 迭代器的架构灵魂：GP 的“万能胶水”
#### 💡 【大厂底层哲学拷问：迭代器到底是什么？】
面向对象（OOP）是把“数据”和“操作（方法）”封装在一起。但 STL 的泛型编程（GP）反其道而行之，它将数据结构（容器）和算法（Algorithm）彻底剥离！
算法的底层源码里根本不知道传进来的是 vector 还是 list，算法只认迭代器！迭代器本质上是一种泛化的智能指针，它通过重载 operator++ 和 operator*，欺骗算法让它以为自己在操作普通数组。这就是 STL 能做到高度解耦和复用的核心机密。
