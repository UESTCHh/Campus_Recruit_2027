...
RunoobTEST
'''
r'''Python 使用反斜杠 \ 转义特殊字符，如果你不想让反斜杠发生转义，可以在字符串前面添加一个 r，表示原始字符串：
...
r'''与 Python 字符串不一样的是，列表中的元素是可以改变的：
...
>>> a
[9, 2, 6]'''
r'''List 内置了有很多方法，例如 append()、pop() 等等，这在后面会讲到。
...
(123, 'runoob', 123, 'runoob')
('abcd', 786, 2.23, 'runoob', 70.2, 123, 'runoob')'''
r'''元组与字符串类似，可以被索引且下标从 0 开始，-1 为从末尾开始的位置，也可以进行截取。
