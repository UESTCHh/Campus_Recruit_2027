...
# 类型注解（Type
# Hints）
# 想象一下，你给朋友寄一个包裹。如果你在包裹上写明
# "易碎品"
# 和
# "向上箭头"，快递员就会知道要小心轻放、正确朝向。类型注解（Type
# Hints） 在编程中就扮演着类似的角色——它是一种为代码添加
# "说明标签"
# 的技术，明确地指出变量、函数参数和返回值应该是什么数据类型。
#
...
# 第二段代码明确指出了
# name
# 应该是字符串类型（str），函数会返回一个字符串（-> str）。
#
# 为什么需要类型注解？
# Python
# 以其动态类型特性而闻名——你不需要提前声明变量的类型，解释器会在运行时自动推断。这虽然灵活，但也带来了问题：
#
...
# 必须使用类型注解吗？
# 不强制。Python
# 仍然是动态类型语言，类型注解是可选的。但强烈推荐使用，特别是大型项目。
#
# 如果注解错了会怎么样？
# 类型检查器会报错，但程序仍然可以运行。注解只是
# "提示"
# 而不是
# "强制"。
#
...
# # 从typing模块导入类型提示相关的类，用于指定函数参数和返回值的类型
# from typing import List, Dict, Optional, Union
#
#
# # 定义函数，处理学生数据并计算平均分数
# # 参数: students - 学生列表，每个学生是包含'name'和'score'的字典
# # 返回: 平均分数（浮点数），如果没有学生则返回None
# def process_students(students: List[Dict[str, Union[str, int]]]) -> Optional[float]:
#     """
#     处理学生数据，计算平均分数
#
...
#         平均分数（浮点数），如果没有学生则返回None
#     """
#     # 检查学生列表是否为空（None、空列表等都视为False）
#     # 如果列表为空，直接返回None，避免除以零的错误
#     if not students:
#         return None
#
#     # 初始化总分为0，用于累加所有学生的分数
#     total = 0
#
#     # 使用for循环遍历学生列表中的每一个学生
#     # 每个student是一个字典，包含'name'和'score'键
#     for student in students:
#         # 通过字典的键'score'获取该学生的分数，并累加到总分中
#         # 这里假设每个字典都包含'score'键，否则会抛出KeyError
#         total += student['score']
#
#     # 计算平均分：总分除以学生人数
#     # 注意：由于之前已经检查过students不为空，这里不会除以零
#     # 返回浮点数结果（Python中/运算符总是返回浮点数）
#     return total / len(students)
#
#
# # 测试数据：创建一个包含3个学生信息的列表
# # 每个学生是一个字典，包含名字(name)和分数(score)
# students_data = [
#     {"name": "Alice", "score": 95},  # 第一个学生：Alice，95分
#     {"name": "Bob", "score": 88},  # 第二个学生：Bob，88分
#     {"name": "Charlie", "score": 92}  # 第三个学生：Charlie，92分
# ]
#
# # 调用process_students函数，传入测试数据，计算平均分
# # 将返回的平均分赋值给变量average
# average = process_students(students_data)
#
# # 使用f-string格式化字符串输出结果
# # {average}会被替换为平均分的值，默认显示小数
# print(f"平均分: {average}")
# 代码执行流程：
# 导入类型提示：使用typing模块提高代码可读性
#
# 定义函数：process_students接收学生列表，返回平均分
#
# 空列表检查：如果列表为空，返回None避免错误
#
# 累加分数：遍历列表，累加所有学生的分数
#
# 计算平均：总分除以学生人数
...
# 平均分: 91.66666666666667
# 注意事项：
# 函数使用了类型提示，但不会强制类型检查，只是提示作用
#
# 如果学生字典缺少
# 'score'
# 键，会抛出KeyError
#
# 如果分数不是数字类型，会抛出TypeError
#
# 函数返回的是浮点数，即使结果是整数也会显示为浮点数（如92
# .0）