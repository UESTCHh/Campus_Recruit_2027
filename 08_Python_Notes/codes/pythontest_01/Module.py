...
# 为此
# Python
# 提供了一个办法，把这些定义存放在文件中，为一些脚本或者交互式的解释器实例使用，这个文件被称为模块。
#
# Python
# 中的模块（Module）是一个包含
# Python
# 定义和语句的文件，文件名就是模块名加上.py
# 后缀。
#
...
#
# Python
# 路径为： ['/root', '/usr/lib/python3.4', '/usr/lib/python3.4/plat-x86_64-linux-gnu', '/usr/lib/python3.4/lib-dynload',
...
#
# 搜索路径时一个解释器会先进行搜索的所有目录的列表。如想要导入模块
...
# 搜索路径是在
# Python
# 编译或安装的时候确定的，安装新的库应该也会修改。搜索路径被存储在
# sys
# 模块中的
# path
# 变量，做一个简单的实验，在交互式解释器中，输入以下代码：
#
# >> > import sys
# >> > sys.path
...
# python
# 解释器的目录（对于脚本的话就是运行的脚本所在的目录）。
#
# 因此若像我一样在当前目录下存在与要引入模块同名的文件，就会把要引入的模块屏蔽掉。
#
# 了解了搜索路径的概念，就可以在脚本中修改
# sys.path
# 来引入一些不在搜索路径中的模块。
#
# 现在，在解释器的当前目录或者
...
#
# 从另一个方面，当你确实知道你在做什么的话，你也可以通过
# modname.itemname
# 这样的表示法来访问模块内的函数。
#
# 模块是可以导入其他模块的。在一个模块（或者脚本，或者其他地方）的最前面使用
# import 来导入一个模块，当然这只是一个惯例，而不是强制的。被导入的模块的名称将被放入当前操作的模块的符号表中。
#
# 还有一种导入的方法，可以使用
...
# 图形库。
#
# 不妨假设你想设计一套统一处理声音文件和数据的模块（或者称之为一个
# "包"）。
#
...
#
# 同样的，这种方法会导入子模块: echo，并且可以直接使用他的
# echofilter()
# 函数:
#
...
#
# Python
# 会进入文件系统，找到这个包里面所有的子模块，然后一个一个的把它们都导入进来。
#
# 但这个方法在
# Windows
# 平台上工作的就不是非常好，因为
...
# *这种语法的时候，就不会导入包
# sound.effects
# 里的任何子模块。他只是把包sound.effects和它里面定义的所有内容导入进来（可能运行__init__.py里定义的初始化代码）。
#
# 这会把
# __init__.py
...
#
# 无论是隐式的还是显式的相对导入都是从当前模块开始的。主模块的名字永远是
# "__main__"，一个Python应用程序的主模块，应当总是使用绝对路径引用。
#
